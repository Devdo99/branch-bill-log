...
export default function ManagerCashiers() {
  ...
  return (
    <AppShell title={`Akses Kasir — ${activeBranch?.name ?? ""}`}>
      <div className="bg-card border rounded-xl p-8 max-w-2xl shadow-card">
        <div className="flex items-center gap-3 mb-4">
          <div className="grid h-12 w-12 place-items-center rounded-lg bg-accent text-accent-foreground">
            <KeyRound className="h-6 w-6" />
          </div>
          <div>
            <h2 className="font-display text-xl font-bold">Login Kasir Tanpa Akun</h2>
        ...
        <ol className="list-decimal pl-5 space-y-2 text-sm">
          ...
          <li>Kasir membuka halaman login → tab <strong>Kasir</strong> → pilih cabang → masukkan PIN.</li>
          <li>Untuk mengganti PIN, hapus dan buat ulang cabang (atau hubungi admin).</li>
        </ol>
        <div className="mt-6 flex gap-2">
          <Link to="/manager/branches"><Button className="bg-gradient-primary"><Building2 className="h-4 w-4 mr-1" /> Kelola Cabang & PIN</Button></Link>
        </div>
      </div>
    </AppShell>
  );
