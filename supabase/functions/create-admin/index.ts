...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Role admin (idempotent)
    const { data: existRole } = await admin.from("user_roles").select("id").eq("user_id", newId!).eq("role", "admin" as any).maybeSingle();
    if (!existRole) {
      await admin.from("user_roles").insert({ user_id: newId!, role: "admin" as any, created_by: user.id });
    }

    // Tambah baris izin (default view_reports = true; lainnya false)
    const { error: pErr } = await admin.from("admin_permissions" as any).upsert({
      branch_id, user_id: newId!, created_by: user.id, view_reports: true,
